'''
Question 1
Write a function that, given an epoch timestamp, returns a datetime object set
to the beginning of that month (so midnight of the first day of the month).
...
We can easily convert an epoch timestamp to a datetime object by using the
fromtimestamp method.

dt = datetime.fromtimestamp(12345678.9)
dt
datetime.datetime(1970, 5, 23, 14, 21, 18, 900000)
Next, we could replace the day, hour, minutes, and seconds with 1 or 0 as
needed, or we could create a new datetime object just picking up the year and
month from this one:
...
Question 2
Write a function that returns the difference in hours between two dates
provided as Python standard ISO formatted strings, rounded to the nearest hour.
For simplicity, assume that these dates do not contain fractional seconds.
...
There are probably many different approaches to this, here I'll explain how I
thought about the problem.

First thing is we'll want to make the total number of seconds into an integer
(we are assuming no fractional seconds).
...
0.5575
We can simply round this number to determine whether we are closer to 0 or 1
for the farctional hour:
...
26
Question 3
Write a function that can be used to consistently format datetime objects into
strings with some default format, but allows the caller to override the
default format.

For example, the default format could be mm/dd/yyyy hh:mm:ss am/pm, but your
function allows itself to be called with some argument that can override that
format.
...
def dt_to_string(dt, fmt='%m/%d/%y %I:%M:%S%p'):
    pass
Notice how I set the default format string in the parameter definition itself -
this way if the function is called without that argument, then the default
will be used.

For these types of parameters though, I prefer to force the function to
specify that fmt argument as a keyword argument, so I can force that this way:
